...
from backend.api.v1.endpoints import auth, entries, fido2, portability, trash
...
app.include_router(
    portability.router, prefix="/api/v1/portability", tags=["Import/Export"]
)
...
async def secure_logging_middleware(request: Request, call_next):
    ...
    logger.info(
        f"[{method}] {url} | IP: {client_ip} | Status: {status_code} | Latency: {process_time:.4f}s"
    )
...
def health_check():
    """Минимальный healthcheck — не раскрывает информацию о системе."""
    return {"status": "ok"}

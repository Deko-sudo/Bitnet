...
from backend.database.schemas import EntryCreateSchema, EntryUpdateSchema, EntryResponseSchema
from backend.core.encryption_helper import EncryptionHelper, EntryFieldsRaw, EntryFieldsEncrypted
...
class EntryService:
    ...
    def create_entry(self, user_id: int, data: EntryCreateSchema) -> PasswordEntry:
        ...
        with self.encryption_helper._operation_key() as key:
            title_str = data.title.get_secret_value()
            title_bytes = bytearray(title_str.encode("utf-8"))
            try:
                # Генерируем Blind Index. Функция уже задеплоена BE1 в crypto_core
    ...
    def get_entry(self, user_id: int, entry_id: int) -> EntryResponseSchema:
        ...
        """
        entry = self.session.query(PasswordEntry).filter(
        ...
        encrypted_fields = EntryFieldsEncrypted(
            title_cipher=entry.title_cipher, title_nonce=entry.title_nonce,
            username_cipher=entry.username_cipher, username_nonce=entry.username_nonce,
            password_cipher=entry.password_cipher, password_nonce=entry.password_nonce,
            url_cipher=entry.url_cipher, url_nonce=entry.url_nonce,
            notes_cipher=entry.notes_cipher, notes_nonce=entry.notes_nonce
        )
        ...
        try:
            ...
            username_sec = SecretStr(decrypted_dict["username"].decode("utf-8")) if decrypted_dict["username"] else None
            url_sec = SecretStr(decrypted_dict["url"].decode("utf-8")) if decrypted_dict["url"] else None
            notes_sec = SecretStr(decrypted_dict["notes"].decode("utf-8")) if decrypted_dict["notes"] else None
    ...
    def update_entry(self, user_id: int, entry_id: int, update_data: EntryUpdateSchema) -> PasswordEntry:
        """
        ...
        if "title" in update_dict:
            blind_index_hex = None
            with self.encryption_helper._operation_key() as key:
                title_str = update_data.title.get_secret_value()
                title_bytes = bytearray(title_str.encode("utf-8"))
                try:
                    # Эмуляция того, что Никита уже сделал коммит
                    if hasattr(self.encryption_helper._crypto, 'generate_blind_index'):
                        blind_index_bytes = self.encryption_helper._crypto.generate_blind_index(
                            bytes(title_bytes), bytes(key)
                        )
                        if blind_index_bytes:
                            blind_index_hex = blind_index_bytes.hex()
            ...
            for field in target_fields:
                if field in update_dict:
                    ...
                    if new_val_sec is not None:
                        cipher_hex, nonce_hex = self.encryption_helper._encrypt_text_field(new_val_sec, key)

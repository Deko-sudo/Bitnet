...
| R007 | `protocol::read_frame` has no read-timeout on the sync `Read` trait | [BITNET-M4](SECURITY_AUDIT_2026-06-09.md#bitnet-m4-protocolread_frame-blocks-indefinitely-partial-fix) | **PARTIAL 2026-06-09**: deadline-based guard added in `handle_one_in_memory` and `dispatch` path. Hard fix requires a tokio-based async transport (Windows Named Pipes do not expose `SetCommTimeouts` through the `Read` trait). Idle connections are now reaped after `MAX_REQUEST_DURATION` seconds instead of holding a thread forever. | v0.2 (async transport) |
